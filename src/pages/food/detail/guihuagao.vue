    ...
    <view class="main-content">
      ...
      <view class="recipe-content-vertical">
        <view class="vertical-text">
          【食材】糯米粉、粘米粉、桂花糖、白糖、清水、桂花少许。<br> 【制作步骤】
          将糯米粉和粘米粉按比例混合，加入白糖和清水调成糊状；桂花糖用温水化开备用；
          将米糊倒入模具，上锅蒸制；蒸至半熟时，在表面撒上桂花糖和桂花；
          继续蒸至完全熟透，取出晾凉；切块装盘即可。
        </view>
      </view>
...
export default {
  ...
  methods: {
    goBack() {
      ...
      uni.navigateBack({
        ...
        fail: () => {
          ...
          uni.redirectTo({
            url: '/pages/food/category/dessert'
          })
        },
        complete: () => {
          setTimeout(() => {
    ...
    startMaking() {
      uni.navigateTo({
        url: '/pages/food/making/guihuagao_making?id=' + this.dish.id
      })
    }
  }
}
